...
It is a prerequisite of the first release rather than a remembered step:
`script/Build.sol` refuses `cutRelease()` while the root is zero
(`InertRegistryRelease`), and refuses the whole release, because a release is
every generated contract frozen into one tag. Deploying an inert registry is
repeatable and stays ungated; freezing one is not, since a frozen snapshot is a
released suite `RainDeployVerifyChain` requires live on every supported network
from then on, and a real root set afterwards adds a second address rather than
