...
/// @param version The version label that is missing.
/// @param deployedAddress The address that should hold it.
error NotDeployedOnNetwork(string network, string version, address deployedAddress);
...
error CodeHashMismatchOnNetwork(
    string network, string version, address deployedAddress, bytes32 expectedCodeHash, bytes32 actualCodeHash
);
...
/// recorded version's derived address carries code with its derived code hash.
///
/// This is the only group that can catch a version that never deployed to a
...
///
/// The matrix is versions by networks and is generated from both, so a new
/// network leaves no version unchecked and a new version is checked on every
/// network from the moment it is recorded. There are deliberately no per-chain
/// or per-version functions to add.
///
...
abstract contract RainDeployVerifyChain is RainDeployVerifyBase {
    /// Checks one derived version against whichever network is currently
    /// selected.
    ...
    function checkDeployedOnNetwork(string memory network, DerivedDeploy memory derived) internal view {
        if (derived.deployedAddress.code.length == 0) {
            revert NotDeployedOnNetwork(network, derived.version, derived.deployedAddress);
        }
        ...
        if (actualCodeHash != derived.bytecodeHash) {
            revert CodeHashMismatchOnNetwork(
                network, derived.version, derived.deployedAddress, derived.bytecodeHash, actualCodeHash
            );
        }
    }

    /// Checks every derived version against every supported network, forking
    /// each network once and checking every version on it.
    ///
    ...
    /// expectation.
    /// @param derived The derivation of every version to check.
    ...
    /// Every recorded version MUST be live, with the code its creation code
    /// produces, on every supported network.
    function testDeployPinsLiveOnEverySupportedNetwork() external {
        checkDeployedOnSupportedNetworks(deriveDeployments(allVersions()));
    }
}
