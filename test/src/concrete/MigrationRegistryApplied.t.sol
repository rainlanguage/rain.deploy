...
/// migration with the moment it was recorded, an unrecorded one with zero,
...
contract MigrationRegistryAppliedTest is Test {
    ...
    /// An unrecorded migration answers zero rather than reverting. This is
    ...
    function testAppliedUnrecordedIsZero(address writer, bytes32 migration) external view {
    ...
    /// A recorded migration answers the timestamp of the block it was recorded
    ...
    /// asked.
    function testAppliedIsTheRecordingTimestamp(address writer, bytes32 migration, uint32 recordedAt, uint32 readAt)
        external
    {
        vm.assume(writer != address(0));
        assumeMigration(migration);
        vm.assume(recordedAt != 0);
        vm.assume(readAt >= recordedAt);
        ...
        vm.warp(readAt);
        assertEq(sRegistry.applied(writer, migration), recordedAt);
    }
    ...
    /// `record` will not write it either, so asking about it would answer zero
    /// forever to a caller that has confused a head for a migration — and that
