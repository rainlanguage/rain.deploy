...
contract LibRainDeploySnapshotTest is Test {
    ...
    /// Where the lib writers are pointed when the writer is what is under test.
    ///
    /// Outside `src/` and `test/`, which are both compiled: a generated lib
    /// imports `../generated/` and `../abstract/`, which resolve from `src/lib`
    /// and nowhere else, so a copy under either root fails the build for every
    /// suite — including the copy a failing test deliberately leaves behind.
    ///
