...
contract LibRainDeploySnapshotTest is Test {
    ...
    /// The directory the escaping write is pointed at, under whatever root it
    /// is handed. Not tag shaped, for the reason
    ...
    /// @param root The record root to generate into.
    /// @param dir The snapshot directory name.
    ...
    /// @param root The record root to walk.
    ...
    /// The header every generated file carries under the org's licence and
    /// copyright, followed by the blank line every writer puts after it.
    ///
    /// Split so `reuse lint` reads this as an expectation rather than as this
    /// file's own license declaration -- an SPDX identifier in a string literal
    /// is indistinguishable from a real one to a line scanner.
    /// @return The prefix.
    function generatedFilePrefix() internal pure returns (string memory) {
    ...
    /// generated-contract list hands them over: `count` of them, in order.
    ///
    /// This repo's own two and then a third, because a third deployed contract
    /// is the scenario the aggregate exists for — the emitted text is asserted
    /// literally below, and a reader has to be able to see what adding one
    /// does to it. Reverts above three rather than quietly emitting a shorter
    /// list, which is the failure a test helper is worst at reporting.
    /// @param count How many contracts.
    /// @return names The contract names.
    function aggregateNames(uint256 count) internal pure returns (string[] memory names) {
        string[] memory all = new string[](3);
        all[0] = "AddressRegistry";
        all[1] = "MigrationRegistry";
        all[2] = "ThirdRegistry";
    ...
    /// The import every aggregate carries whatever it aggregates.
    string constant AGGREGATE_SUITE_IMPORT = "import {DeploySuite} from \"../abstract/RainDeploySuitesBase.sol\";\n\n";

    /// The aggregate's text from its title down to the line that opens
    /// `releasedSuites`. The same whatever it aggregates, so the assertions
    /// below are about the body.
    string constant EXPECTED_AGGREGATE_HEADER = "/// @title LibReleasedSuites\n"
    ...
    /// else.
    ///
    /// An aggregate that imports one released lib fewer than the list it was
    /// emitted from is the hand-written concatenation this replaces: it
    /// compiles, it is read by nothing, and the contract left out of it has no
    /// declared release at all.
    function testAggregateImportBlockImportsEveryReleasedLib() external pure {
        assertEq(LibRainDeploySnapshot.aggregateImportBlock(aggregateNames(0)), AGGREGATE_SUITE_IMPORT);

        assertEq(
            LibRainDeploySnapshot.aggregateImportBlock(aggregateNames(1)),
            string.concat(
                AGGREGATE_SUITE_IMPORT,
                "import {LibAddressRegistryReleased} from \"./LibAddressRegistryReleased.sol\";\n\n"
            )
        );

        assertEq(
            LibRainDeploySnapshot.aggregateImportBlock(aggregateNames(3)),
            string.concat(
                AGGREGATE_SUITE_IMPORT,
                "import {LibAddressRegistryReleased} from \"./LibAddressRegistryReleased.sol\";\n\n",
    ...
    /// A repo with no generated contracts MUST still emit an aggregate, and it
    /// MUST return an empty array.
    ///
    /// That is the state of a deploy repo whose declaration is not written yet,
    /// and the aggregate is imported by ordinary source — so a repo that could
    /// not emit one could not build to the point of declaring anything. It is
    /// also the only shape with no locals to sum, which is why it is a case
    ...
    /// list it was handed.
    ///
    /// Handed a list that is NOT this repo's, so a writer that emitted the
    /// declaration it happened to find rather than the list it was given says
    /// so. Removed BEFORE the assertions run, because forge-std assertions
    /// revert: removing afterwards removes in every case except a failure,
    /// which is the only case where the tree is dirty and the one this test
    /// exists to report.
    function testWriteReleasedSuitesAggregateWritesTheLibAtItsPath() external {
        ...
        assertEq(
            ...
            string.concat(
                generatedFilePrefix(),
                LibRainDeploySnapshot.aggregateImportBlock(names),
                LibRainDeploySnapshot.aggregateLibraryBlock(vm, names)
            )
        );
    }

    /// This repo's COMMITTED aggregate MUST be what the generator emits for the
    /// contracts it names.
    ///
    ...
    /// contracts this repo generates is
    /// `GeneratedSnapshotShapeTest.testEverySnapshotIsInTheReleasedAggregate`.
    /// Neither half is worth anything alone: a file consistent with a list that
    /// left a contract out is exactly the defect being removed.
    ///
    /// Writes nothing, so it cannot collide with the writer above.
    function testTheCommittedAggregateIsWhatTheGeneratorEmits() external view {
        string memory path = LibRainDeploySnapshot.pathForLib(LibRainDeploySnapshot.RELEASED_SUITES_LIBRARY);
        string memory committed = vm.readFile(path);
        string[] memory names = LibReleasedSuitesAggregate.declaredContractNames(vm, committed);

        assertEq(path, "src/lib/LibReleasedSuites.sol");
        assertEq(
            committed,
            string.concat(
                generatedFilePrefix(),
